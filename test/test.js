...
contract('Testing mainnet', ([acc]) => {
  ...
  describe('Compound', () => {
    ...
    it('getting cDai...', async () => {
      const cDaiBalanceBefore = await cDai.methods.balanceOf(acc).call()
      const daiBalance = await dai.methods.balanceOf(acc).call()

      await dai.methods.approve(legos.compound.cDAI.address, daiBalance).send({from: acc})
      await cDai.methods.mint(daiBalance).send({from: acc, gasLimit: 4000000})

      const cDaiBalanceAfter = await cDai.methods.balanceOf(acc).call()
      expect(Number(cDaiBalanceAfter)).to.be.above(Number(cDaiBalanceBefore))
    })
  })
})
